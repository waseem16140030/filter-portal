  ...
  return (
    <>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <div className='flex items-center justify-between border-be pli-5 plb-3 gap-2'>
          <Title hidden />
          <Description hidden />
          <i className='bx-search min-is-6' />
          <CommandInput value={searchValue} onValueChange={setSearchValue} />
          <span className='text-textDisabled'>[esc]</span>
          <i className='bx-x min-is-6 cursor-pointer' onClick={() => setOpen(false)} />
        </div>
        <CommandList>
          {searchValue ? (
