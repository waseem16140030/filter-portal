...
const PlanDetails = ({ data, pricingPlan }: Props) => {
  return (
    <CardContent
      className={classnames('relative border rounded pbs-[3.75rem] flex flex-col gap-5', {
      ...
      <div className='flex flex-col gap-4'>
        {data?.planBenefits.map((item: string, index: number) => (
          <div key={index} className='flex items-center gap-2'>
            <CustomAvatar color='primary' skin='light' size={20}>
              <i className='bx-check text-base text-primary' />
            </CustomAvatar>
