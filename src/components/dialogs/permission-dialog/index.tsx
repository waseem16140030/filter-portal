  ...
  return (
    <>
      ...
      <DialogActions className='max-sm:flex-col max-sm:gap-4 justify-center pbs-0 sm:pbe-16 sm:pli-16'>
        <Button type='submit' variant='contained' onClick={handleClose} className='max-sm:is-full'>
          Create Permission
        </Button>
        <Button onClick={handleClose} variant='tonal' color='secondary' className='max-sm:mis-0 max-sm:is-full'>
          Discard
        </Button>
      </DialogActions>
    </>
...
const EditContent = ({ handleClose, data }: EditProps) => {
  return (
    <DialogContent className='overflow-visible pbs-0 sm:pli-16'>
      ...
      <div className='flex max-sm:flex-col sm:items-end gap-4 mbe-2'>
        <CustomTextField
          fullWidth
          size='small'
          defaultValue={data}
          ...
          placeholder='Enter Permission Name'
        />
        <Button variant='contained' onClick={handleClose} className='max-sm:is-full'>
          Update
        </Button>
      </div>
      <FormControlLabel control={<Checkbox />} label='Set as core permission' />
